...
class ManageEmployeesListView extends StatelessWidget {
        ...
        return ListView(
          ...
          children: [
            ...
            if (showSearch) ...[
              ...
              ...state.searchResults.map((user) => _buildUserCard(context, user, isSearchResult: true)),
              const SizedBox(height: 24),
            ],

            // Current Employees Section
            Text(
              context.tr('currentEmployeesTitle', defaultValue: 'Current Employees'),
              style: const TextStyle(
                color: ColorManager.creasedKhaki,
                fontSize: 16,
                fontWeight: FontWeight.bold,
            ...
            if (list.isEmpty)
              Padding(
                ...
                child: Center(
                  child: Text(
                    context.tr('noEmployeesFound', defaultValue: 'No employees added yet.'),
                    style: const TextStyle(color: Colors.white54, fontSize: 14),
                  ),
                ),
              )
            else
              ...list.map((emp) => _buildUserCard(context, emp, isSearchResult: false)),
          ],
        );
      },
    );
  ...
  Widget _buildUserCard(BuildContext context, UserModel user, {required bool isSearchResult}) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: isSearchResult ? ColorManager.wasabi.withOpacity(0.5) : ColorManager.emeraldGreen.withOpacity(0.2),
        ),
      ),
      child: Row(
        children: [
          CircleAvatar(
            ...
            child: const Icon(Icons.person_rounded, color: ColorManager.egyptianEarth),
          ),
                ...
                if (isEmp) ...[
                  ...
                  Container(
                    ...
                    child: Text(
                      ...
                      style: const TextStyle(color: ColorManager.wasabi, fontSize: 11, fontWeight: FontWeight.bold),
                    ),
                  )
                ]
              ],
            ...
            if (canManageEmployees) {
              return ElevatedButton(
                style: ElevatedButton.styleFrom(
                  backgroundColor: isSearchResult && !isEmp ? ColorManager.wasabi : ColorManager.egyptianEarth,
                ...
                onPressed: () {
                  showDialog(
                    ...
                    builder: (_) => EditEmployeeDialog(
                      user: user,
                      places: places,
                      onSave: ({required assignedPlaceIds, required permissions}) {
                        cubit.saveEmployee(
                          employeeId: user.id,
                ...
                child: Text(
                  isEmp ? context.tr('edit', defaultValue: 'Edit') : context.tr('add', defaultValue: 'Add'),
                  style: TextStyle(
                    color: isSearchResult && !isEmp ? Colors.black : Colors.white,
                    fontWeight: FontWeight.bold,
                  ),
                ),
              );
